# 🚀 AI Prompt Generator & Text Transformer Studio

**MiniMax H3 & LTX-Video 2.5 & KREA 2 & Text Revision Studio v1.3**  
...
- **`● 텍스트 가공 도구 (Text Transformer Studio v1.3)`**:
  - **듀얼 AI 엔진 지원**: Google Gemini API & 로컬 LLM (LM Studio / Ollama / vLLM, `http://localhost:1234/v1/chat/completions`) 비검열 무제한 모드
  ...
  - **대용량 청크 분할 처리**: 40줄 이상 대용량 텍스트 자동 순차 분할 및 진행률 애니메이션
  - **일괄 가공(Batch Processing)** 및 **히스토리 자동 저장**

---

### 2. ⚡ 듀얼 엔진 & 프라이버시
- **Google Gemini API**: `Gemini 3.5 Flash`, `Gemini 3.6 Flash`, `Gemini 2.5 Pro` 지원, 5대 세이프티 필터 전체 `BLOCK_NONE` 적용으로 제한 없는 예술적/시네마틱 프롬프트 생성.
...
3. 우측 상단 `Gemini API 키 설정`에 Google AI Studio에서 무료로 발급받은 API Key를 저장하거나, `텍스트 가공 도구`에서 `로컬 LLM`을 선택하여 바로 사용하세요!

---

## 💻 지원 환경
- **웹 브라우저**: Google Chrome, Microsoft Edge, Whale, Safari, Firefox 등 모든 최신 웹 브라우저 (PC & Mobile 완벽 대응)
- **별도 요구사항**: 설치 불필요 (Zero-dependency)
